...
void CIriFitnessFunction::SimulationStep(unsigned int n_simulation_step, double f_time, double f_step_interval)
{
	...
	/* Fitness function experiment 1 */
	//double fitness = maxSpeedEval * sameDirectionEval * (leftSpeed * rightSpeed) * ((battery[0]+blueBattery[0]+redBattery[0])/3);
	
	/*Fitness function experiment 2 */
	double fitness = maxSpeedEval * sameDirectionEval * (leftSpeed * rightSpeed) * battery[0] * blueBattery[0] * redBattery[0];
